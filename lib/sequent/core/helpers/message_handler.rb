    ...
    module Helpers
      ...
      module MessageHandler
        module ClassMethods
          def on(*message_classes, &block)
            message_classes.each do |message_class|
              message_mapping[message_class.to_s] ||= []
              message_mapping[message_class.to_s] << block
            end
          end
          ...
          def handles_message?(message)
            message_mapping.keys.include? message.class.to_s
          end
        end
        ...
        def handle_message(message)
          handlers = self.class.message_mapping[message.class.to_s]
